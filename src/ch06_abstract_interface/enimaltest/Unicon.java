...
public class Unicon extends Animal implements Bird, Horse {
    ...
    @Override
    public void fly() {
        String messsage =this.name+"이(가) 시속"+ Bird.speed+"의 속도로 날아 갑니다";
        System.out.println(messsage);
    }

    @Override
    public void run() {
        String messsage =this.name +"이(가) 시속"+Horse.speed+"의 속도로 달려 갑니다";
        System.out.println(messsage);
    }

    @Override
    public void eat() {
        String messsage =this.name+"이(가)"+ this.feed+("를 먹습니다");
        System.out.println(messsage);
    }
